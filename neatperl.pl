...
# 0.5  2019/09/03  BEZROUN    Vhecking of re-formatted script via bash -n before writing formatted version to a file implemented
#=========================== START =========================================================
#=== Start
   use v5.10;
#  use Modern::Perl;
...
# INTERESTING, VERY NEAT IDEA: you can switch on tracing from particular line of source ( -1 to disable)
   ...
   %delim=('if'=>1,'for'=>1, 'case'=>1, 'switch'=>1, 'while'=>1,'until'=>1);
...
my $scan_text=$_[0];
      ...
      if( $single_quote%2==0 && $double_quote%2==0 ){
         unless( exists($delim{$first_word}) ){
            if( $curve_br>0 ){
               $inbalance ='{';
