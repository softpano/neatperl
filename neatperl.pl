...
# 0.72  2019/10/04  BEZROUN    commit_source made more plexible by specifing work directory
...
#
   ...
   $CodeLinesNo=$TotalSubNo=0; # code metrics
   #$top=0; $stack[$top]='';
   $lineno=0;
...
sub code_metric
{
    out("Total lines without comments: $CodeLinesNo; Max nesting: $MaxNest; Internal subs: $TotalSubNo");
    if( $MaxNest>4) {
