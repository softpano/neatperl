...
# 0.5  2019/09/03  BEZROUN    Checking of re-formatted script via bash -n before writing formatted version to a file implemented
...
# INTERESTING, VERY NEAT IDEA: you can switch on tracing from particular line of source ( -1 to disable)
   ...
   $write_pipe=0;
   $readability_plus=0;
   %keyword=('if'=>1,'while'=>1,'unless'=>1, 'until'=>1,'for'=>1,'foreach'=>1);
